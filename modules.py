...
from utils import region_partition, region_reverse


# AB-MIL
class ABMIL(nn.Module):
    def __init__(self, input_dim=512, n_classes=2, dropout=True, act='relu'):
...
class RRTMIL(nn.Module):
    def __init__(self, input_dim=512, n_classes=2, dropout=True, act='relu', epeg_k=15, crmsa_k=3):
...
#WSI-Feature-Generator
class Generator(nn.Module):
    def __init__(self, input_dim=512, epeg_k=15, crmsa_k=3):
    ...
    def forward(self, x):
        ...
        A = torch.transpose(A, -1, -2)
        A = F.softmax(A, dim=-1)
        bag_feature = torch.mm(A, feature)
...
class Generatorvis(nn.Module):
    def __init__(self, input_dim=512, epeg_k=15, crmsa_k=3):
        super(Generatorvis, self).__init__()
        self.L = 512
        self.D = 128
        self.K = 1
    ...
    def forward(self, x):
        ini_feature = self.feature(x)
        rrt_feature = self.rrt(ini_feature)

        return ini_feature, rrt_feature


class RRTMILvis(nn.Module):
    def __init__(self, input_dim=512, n_classes=2, dropout=True, act='relu', epeg_k=15, crmsa_k=3):
        super(RRTMILvis, self).__init__()
        self.L = 512
        self.D = 128
        self.K = 1
        self.feature = [nn.Linear(input_dim, 512)]
        self.rrt = RRTEncoder(input_dim=input_dim, epeg_k=epeg_k, crmsa_k=crmsa_k)
