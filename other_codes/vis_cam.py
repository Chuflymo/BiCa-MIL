...
for str_id in str_lst:
    ...
    cam_1 = torch.load(fr"D:\DATA\PLIP_MIL\draw_pictures\weights\adapter_cam\tumor{str_id}_cam.pt")
    median_val = torch.median(torch.abs(cam_1)).item()
    scale = 1.0 / max(median_val, 1e-10)
    cam_1 = torch.torch.nn.functional.softmax(cam_1*scale,dim=0)
    cam_1 = cam_1[1]
    cam_1 = cam_1.cpu().numpy()
    ...
    A_roi,coords_roi,cam_roi = screen_coords(data_1,loc_1,(left,top),(right,down),cam=cam_1)
    print(">>>>>>>>>>cam_roi>>>>>>>>>>>>")
    ...
    for _idx, _coord in enumerate(coords_roi_rel):
        ...
        if cam_roi[_idx] != 0:
            ...
            int(_coord[0] * scale_level_ratio[0]):int((_coord[0] + patch_size) * scale_level_ratio[0]), :] = 1 - \
                                                                                                             cam_roi[
                                                                                                                 _idx]
    ...
    blended_image = (alpha_mat_attn * img + alpha_mat_cam * img + (1 - alpha_mat_attn) * heatmap_attn + (
                1 - alpha_mat_cam) * heatmap_cam)
    ...
    # if save_figure:
    path = os.path.join('D:/DATA/PLIP_MIL/draw_pictures/heatmap/adapter_cam_map',f_id+'_roi'+str(roi)+'.png')
