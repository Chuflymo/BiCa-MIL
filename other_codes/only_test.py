...
def main(seed, num_samples):
    ...
    PlipAdapter_model = torch.load(
        fr"D:\DATA\PLIP_MIL\plip_adapter_mil_model\l1_plip_adapter_mil_model_{seed}_{num_samples}.pt").to(device)
    best_auc_score = 0.0
    best_loss = float('inf')
...
if __name__ == '__main__':
    sample_lst = [10]
    seed_lst = [2024]
    for num_samples in sample_lst:
        for seed in seed_lst:
            print(f'Seed: {seed}, ')
            print(f'{num_samples} samples')
            main(seed=seed, num_samples=num_samples)