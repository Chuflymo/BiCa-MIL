...
def adapter_vis(model_save_path,raw_feature_path, feature_save_path):
    ...
    Adapter_model = torch.load(model_save_path).to(device)
    Adapter_model.eval()
    with torch.no_grad():
        directory = str(raw_feature_path)
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.endswith('.pt'):
                    ...
                    draw_data_feature = draw_data['features'].to(device)
                    plip_feature, rrt_feature = Adapter_model(draw_data_feature)
                    plip_save_path = os.path.join(feature_save_path, f'{name}_plip_feature.pt')
                    rrt_save_path = os.path.join(feature_save_path, f'{name}_adapter_rrt_feature.pt')
                    torch.save(plip_feature, plip_save_path)
                    torch.save(rrt_feature, rrt_save_path)

def ab_vis(model_save_path,raw_feature_path, feature_save_path):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    RRT_model = torch.load(model_save_path).to(device)
    RRT_model.eval()
    with torch.no_grad():
        directory = str(raw_feature_path)
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.endswith('.pt'):
                    ...
                    draw_data = torch.load(file_path)
                    draw_data_feature = draw_data['features'].to(device)
                    rrt_feature = RRT_model(draw_data_feature)
                    rrt_save_path = os.path.join(feature_save_path, f'{name}_ab_rrt_feature.pt')
                    torch.save(rrt_feature, rrt_save_path)
...
def main():
    directory_1 = r'D:\DATA\PLIP_MIL\draw_pictures\rrt_feature'
    for root, dirs, files in os.walk(directory_1):
        for file in files:
            if file.endswith('.pt'):
                ...
                coords = patch['coords'][:]
                plip_tsne = tsne(plip_feat, coords, rf"D:\DATA\PLIP_MIL\draw_pictures\lesion_annotations\{name}.xml",
                                 draw_legend=False)
                if plip_tsne is not None:
                    plip_tsne.get_figure().savefig(rf'D:\DATA\PLIP_MIL\draw_pictures\rrt_png\{name}_rrt.png', dpi=300,
                                                   bbox_inches='tight')

if __name__ == '__main__':
    # get features for visualization
    ab_vis(model_save_path=r"D:\DATA\PLIP_MIL\ab_mil_model\l1_ab_mil_2024.pt",
            feature_save_path=r'D:\DATA\PLIP_MIL\draw_pictures\rrt_feature',
            raw_feature_path=r"D:\DATA\PLIP_MIL\RAW_PLIP_features\train_and_val")

    main() # Modify the file save path in the function
